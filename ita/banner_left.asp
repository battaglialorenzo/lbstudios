<table border="0" cellpadding="0" cellspacing="0" align="center">
  ...
  <%
  set rsbl=cn.execute("select * from [93.186.241.74].[toplegalinternational].dbo.link where banner<>0 and dovebanner='left' and idlingua="&lingua&" order by ordlink ")
 if rsbl.eof then
 set rsbl=cn.execute("select * from [93.186.241.74].[toplegalinternational].dbo.link where banner<>0 and dovebanner='left'  order by ordlink ")
 end if
...
target="_blank"
 ...
 %><tr>
  ...
  <script language="JavaScript1.1">generatorflash('/upload/banner/<%=imgbanner%>','<%=width%>','<%=height%>','ffffff','7,0,0,0','7','','flash');</script>
  <%else%>
